...
class VIPR(nn.Module):
    ...
    def __init__(self,genomes,means,vars,pop_size,var_dist="LogNormal"):
        super().__init__()
        ...
        if var_dist == "Gamma":
            log_alphas = torch.tril(2*torch.log(means) - torch.log(vars),diagonal=-1)
            log_betas = torch.tril(torch.log(means) - torch.log(vars),diagonal=-1)
            self.theta = nn.Parameter(torch.stack([log_alphas,log_betas]))
        elif var_dist == "LogNormal":
    ...
    def get_var_dist(self,theta):

        if self.var_dist == "Gamma":
    ...
    def multisample_ELBO_reparam(self,batch_size,detach=False):
        ...
        for i in range(batch_size):
            ...
            log_prior = self.logprior(Z)
            log_ll = self.loglikelihood(Z)
            log_q = self.logvariational(Z)
    ...
    def ELBO_reparam(self,batch_size,detach=False):
        ...
        for _ in range(batch_size):
            ...
            log_ll = self.loglikelihood(Z)
            log_q = self.logvariational(Z)
            ELBO = ELBO + (log_ll + log_prior - log_q)/batch_size
    ...
    def record_metrics(self,iter,test_batch_size):
        ...
        self.thetas.append(self.theta.detach().numpy())
    ...
    def learn(self,batch_size,iters,alpha,method="reparam",
        ...
        for it in range(iters):
            ...
            if torch.any(torch.isnan(self.theta.grad)):
