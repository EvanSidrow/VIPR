...
from treePriors import ConstantCoalescent, SkyrideCoalescent, FixedPopCoalescent
...
class VBPI(nn.Module):
    EPS = 1e-40
    CoalescentModel = {'fixed_pop': FixedPopCoalescent,
                       'constant': ConstantCoalescent,
    ...
    def __init__(self, taxa, rootsplit_supp_dict, subsplit_supp_dict, data, pden, subModel, emp_tree_freq=None,
                 sample_info=None, psp=True, mu_0=-0.0049, sigma_0=2.148, coalescent_type='fixed_pop',
    ...
    def learn(self, stepsz, maxiter=100000, test_freq=1000, lb_test_freq=1000, anneal_freq=20000, anneal_rate=0.75, n_particles=10,
              init_inverse_temp=0.001, warm_start_interval=50000, method='vimco', save_to_path=None, max_time=12.0):
        lbs, lls, lps, root_ages = [], [], [], []
        test_kl_div, test_lb, run_times, its = [], [], [], []
        ...
        for it in range(1, maxiter+1):
            ...
            if (sum(run_times)/3600) > max_time:
            ...
            if it % test_freq == 0:
                ...
                test_lb.append(self.lower_bound(n_particles=1,n_runs=1000))
        ...
        return test_lb, test_kl_div, run_times, its
