...
class FixedPopCoalescent(BaseCoalescent):
    def __init__(self, *args, log_pop_size = math.log(5.0), **kwargs):
        super().__init__()
        self.log_pop_size = log_pop_size
    ...
    def loglikelihood(self, log_pop_size):
        eff_pop_size = math.exp(log_pop_size)
    ...
    def forward(self, log_pop_size, monitor_prior=True):
        log_coalescent_prior = self.loglikelihood(log_pop_size)
        if not monitor_prior:
            return log_coalescent_prior, None
        else:
            return log_coalescent_prior, log_coalescent_prior - log_pop_size

    def sample_pop_size(self, *args, n_particles=1):
        return self.log_pop_size, 0.0
